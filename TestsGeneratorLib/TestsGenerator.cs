...
namespace TestsGeneratorLib
{
    public static class TestsGenerator
    {
        ...
        private const string DefaultBody = "Assert.Fail(\"autogenerated\");";

        private const string AdditionalBody = "int expected = 0;" +
        ...
        private static string CreateTest(TypeDeclarationSyntax classDeclaration)
        {
            ...
            var @namespace = SyntaxFactory.NamespaceDeclaration(
                    SyntaxFactory.QualifiedName(
                        SyntaxFactory.IdentifierName(classNamespaceName ?? "Kra" + ".Test"),
                        SyntaxFactory.IdentifierName(
                            SyntaxFactory.Identifier(
                                SyntaxFactory.TriviaList(),
                                "Test",
        ...
        private static List<MethodDeclarationSyntax> AddTestMethods(TypeDeclarationSyntax classDeclaration)
        {
            ...
            foreach (var method in methods)
            {
                ...
                {
                    var value = SyntaxFactory.EqualsValueClause(SyntaxFactory.LiteralExpression(
                        SyntaxKind.NullLiteralExpression));
                    if (parameter.Default != null)
                    {
                        value = SyntaxFactory.EqualsValueClause(parameter.Default.Value);
                    }

                    arrange.Add(SyntaxFactory.LocalDeclarationStatement(
                            SyntaxFactory.VariableDeclaration(
                                    SyntaxFactory.IdentifierName(
                                        Namespace + Namespace + Namespace + parameter.Type.ToFullString()))
                                .WithVariables(
                                    SyntaxFactory.SingletonSeparatedList(
                                        SyntaxFactory.VariableDeclarator(
